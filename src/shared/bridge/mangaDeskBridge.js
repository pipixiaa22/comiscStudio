...
export const mangaDeskBridge = {
    ...
    relocateSource: async (id, input) => unwrap(await bridge().relocateSource(id, input)),
    ...
    chooseSourceReplacement: async input => bridge().chooseSourceReplacement(input),
    ...
    voice: {
        ...
        takeUrl: async input => unwrap(await bridge().voice.takeUrl(input)),
    ...
    settings: {get: async () => unwrap(await bridge().settings.get()), save: async input => unwrap(await bridge().settings.save(input)), chooseMediaTool: async key => unwrap(await bridge().settings.chooseMediaTool(key)), mediaToolStatus: async () => unwrap(await bridge().settings.mediaToolStatus()), installMediaTools: async () => unwrap(await bridge().settings.installMediaTools()), onMediaToolProgress: callback => bridge().settings.onMediaToolProgress(callback)}
    ,
    assistant: {
        ...
        cacheList: async () => unwrap(await bridge().assistant.cacheList()),
        cacheRemove: async name => unwrap(await bridge().assistant.cacheRemove(name)),
        cachePrune: async () => unwrap(await bridge().assistant.cachePrune()),
