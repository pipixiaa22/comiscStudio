...
const contentUpdate = (state, mutate, { recordHistory = true } = {}) => {
  ...
  mutate(project)
  return { ...state, project, current: project.workspace.currentBlockId, dirty: true, revision: state.revision + 1, undo: recordHistory ? appendHistory(state.undo, state.project) : state.undo, redo: recordHistory ? [] : state.redo, activeTextEdit: null }
}

const updateWorkspace = (state, workspace) => ({ ...state, project: { ...state.project, workspace: { ...state.project.workspace, ...workspace } }, current: workspace.currentBlockId || state.current, dirty: true, revision: state.revision + 1, activeTextEdit: null })
...
function updateText(state, action) {
  ...
  blocks[blockIndex] = { ...oldBlock, text: action.text, status: { ...oldBlock.status, scriptDone: false }, updatedAt: timestamp }
  return {
    ...state,
...
export function projectReducer(state, action) {
  switch (action.type) {
    ...
    case 'TOGGLE_FAVORITE': {
      ...
      return contentUpdate(state, project => { const index = project.favorites.findIndex(entry => entry.sourceId === action.sourceId); if (index >= 0) project.favorites.splice(index, 1); else project.favorites.push({ sourceId: action.sourceId, createdAt: now() }) })
    }
    ...
    case 'REMOVE_BASKET_ITEM': {
      ...
      return contentUpdate(state, project => { project.scratchBasket = project.scratchBasket.filter(item => item.id !== action.itemId).map((item, order) => ({ ...item, order })) })
    }
    case 'REORDER_BASKET': {
      ...
      if (from < 0 || from === to) return state
      return contentUpdate(state, project => { const [item] = project.scratchBasket.splice(from, 1); project.scratchBasket.splice(to, 0, item); project.scratchBasket = project.scratchBasket.map((entry, order) => ({ ...entry, order })) })
    }
    case 'CLEAR_BASKET': return state.project.scratchBasket.length ? contentUpdate(state, project => { project.scratchBasket = [] }) : state
    ...
    case 'COMMIT_TEXT_HISTORY': return { ...state, activeTextEdit: null }
    case 'UNDO': return state.undo.length ? { ...undoHistory(state), dirty: true, revision: state.revision + 1, activeTextEdit: null } : state
    case 'REDO': return state.redo.length ? { ...redoHistory(state), dirty: true, revision: state.revision + 1, activeTextEdit: null } : state
    case 'SAVE_STARTED': return { ...state, saveStatus: 'saving', saveError: null }
    case 'SAVE_SUCCEEDED': return action.revision === state.revision ? { ...state, savedRevision: action.revision, dirty: false, saveStatus: 'saved', saveError: null } : { ...state, savedRevision: Math.max(state.savedRevision, action.revision), saveStatus: 'dirty', saveError: null }
    case 'SAVE_FAILED': return { ...state, saveStatus: 'error', saveError: action.error || '保存失败' }
