import { BookOpen, Check, Download, FolderOpen, Save, PanelsTopLeft, Clapperboard, Plus, Library, MoreHorizontal, ChevronDown, Settings } from 'lucide-react'
import {useEffect, useState} from 'react'
import { Button } from '../components/ui/button'
import { ProjectProgress } from '../features/project/components/ProjectProgress'

export function AppHeader({ project, saveStatus, view, onViewChange, onSave, onImport, onAppendSource, onProjectCenter, onExport, onAssistant, onSettings, narrationMode, onNarrationMode, onMediaTab }) {
  ...
  return <header className="flex h-14 shrink-0 items-center border-b border-border bg-background px-5">
    <div className="flex items-center gap-2 text-xl font-black"><BookOpen className="h-5 w-5 text-foreground" />Manga<span>Desk</span></div>
    <div className="ml-5 min-w-0 border-l border-border pl-5"><div className="truncate text-sm text-foreground">{project?.name || '未命名项目'}</div>{project && <div className="flex items-center gap-1.5 text-xs text-muted-foreground"><i className={`h-1.5 w-1.5 rounded-full ${saveStatus.status === 'error' ? 'bg-destructive' : 'bg-foreground'}`}/>{status}</div>}</div>
    ...
    <div className="ml-auto flex items-center gap-2">
      ...
      {project && <div className="relative"><Button aria-label="更多项目操作" title="更多项目操作" variant="ghost" size="icon" onClick={() => { setMoreMenuOpen(value => !value); setSourceMenuOpen(false) }}><MoreHorizontal className="h-5 w-5"/></Button>{moreMenuOpen && <div aria-hidden className="fixed inset-0 z-40" onClick={() => setMoreMenuOpen(false)}/>}{moreMenuOpen && <div className="absolute right-0 top-full z-50 mt-2 w-44 rounded-md border border-border bg-popover p-1 shadow-xl"><button className="flex w-full items-center gap-2 rounded px-3 py-2 text-left text-sm hover:bg-accent" onClick={() => { setMoreMenuOpen(false); onProjectCenter() }}><Library className="h-4 w-4"/>项目中心</button><button className="flex w-full items-center gap-2 rounded px-3 py-2 text-left text-sm hover:bg-accent" onClick={() => { setMoreMenuOpen(false); onAssistant() }}><Clapperboard className="h-4 w-4"/>剪映辅助</button><button className="flex w-full items-center gap-2 rounded px-3 py-2 text-left text-sm hover:bg-accent" onClick={() => { setMoreMenuOpen(false); onSettings() }}><Settings className="h-4 w-4"/>设置</button><div className="my-1 border-t border-border"/><p className="px-3 py-1 text-[10px] text-muted-foreground">旁白模式</p><button className="flex w-full items-center gap-2 rounded px-3 py-2 text-left text-sm hover:bg-accent" onClick={() => { setMoreMenuOpen(false); onNarrationMode('text') }}><Check className={`h-4 w-4 ${narrationMode === 'text' ? 'text-foreground' : 'text-transparent'}`}/>文字文案</button><button className="flex w-full items-center gap-2 rounded px-3 py-2 text-left text-sm hover:bg-accent" onClick={() => { setMoreMenuOpen(false); onNarrationMode('voice') }}><Check className={`h-4 w-4 ${narrationMode === 'voice' ? 'text-foreground' : 'text-transparent'}`}/>真人录音</button></div>}</div>}
    </div>
  </header>
}
