...
export const BlockEditor = memo(function BlockEditor({
    ...
    return <section className="flex min-h-0 flex-col border-r border-border bg-background">
        <div className="flex min-h-14 flex-wrap items-center gap-2 border-b border-border px-5 py-2"><ImageIcon className="h-4 w-4 text-foreground"/><b className="text-sm">文案编辑器</b>
        ...
        <ScrollAreaBox className="min-h-0 flex-1">
            <div className="mx-auto max-w-2xl space-y-3 p-5">{blocks.filter(matches).map((entry, visibleIndex) => {
                ...
                return <Card key={entry.id}
                             className={entry.id === block?.id ? 'border-foreground bg-selected' : 'border-border'}>
                    ...
                    {expanded && <CardContent><textarea value={entry.text}
                                                        ...
                                                        className="min-h-40 w-full resize-y rounded-lg border border-input bg-background p-4 text-[15px] leading-7 outline-none focus:border-foreground"/>
        ...
        {importOpen && <div className="absolute inset-0 z-30 flex items-center justify-center bg-slate-950/75 p-6">
            <Card className="w-full max-w-2xl border-slate-600 shadow-2xl"><CardContent className="space-y-4 p-5"><div className="flex items-center"><div><h2 className="font-bold">导入整篇文稿</h2><p className="mt-1 text-xs text-slate-400">保留原文与中文标点；不会自动按句号拆分。</p></div><button className="ml-auto text-sm text-slate-400 hover:text-white" onClick={() => setImportOpen(false)}>关闭</button></div>
                ...
                <textarea autoFocus value={importText} onChange={event => setImportText(event.target.value)} placeholder="粘贴全文…" className="min-h-64 w-full rounded-lg border border-input bg-background p-3 leading-6 outline-none focus:border-foreground"/>
