...
export function BlockStatusPanel({ projectId, narrationMode, block, index, total, sourcesById, basket, commands, undoCount, redoCount, onLocateAsset, selectedAssetId, onSelectAsset }) {
  ...
  return <aside className="min-h-0 overflow-auto bg-[#181c25] p-4" onDragOver={event => event.preventDefault()} onDrop={dropBasket}>
    ...
    <Card className="mt-3"><CardContent className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">{statuses.map(([key, label]) => <label key={key} className="flex items-center gap-2"><input type="checkbox" checked={narrationMode === 'voice' && key === 'voiced' ? (block.voice?.narrationRequired === false || Boolean(block.voice?.activeTakeId)) : Boolean(block.status[key])} disabled={narrationMode === 'voice' && key === 'voiced'} onChange={event => commands.setBlockStatus(block.id, key, event.target.checked)} />{narrationMode === 'voice' && key === 'voiced' ? '录音' : label}</label>)}</div>
    ...
    <BlockAssetList assets={block.assets} sourcesById={sourcesById} selectedAssetId={selectedAssetId} onSelect={onSelectAsset} onLocate={onLocateAsset} onRemove={assetId => commands.removeAsset(block.id, assetId)} onMove={(assetId, toIndex) => commands.moveAsset(block.id, assetId, toIndex)} />
