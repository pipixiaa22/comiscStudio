...
export function ProjectCenter({onOpen, onClose}) {
    ...
    return <main className="min-h-0 flex-1 overflow-auto bg-background p-6"><section className="mx-auto max-w-4xl"><div className="flex items-center"><div><h1 className="text-xl font-bold">项目中心</h1><p className="mt-1 text-sm text-muted-foreground">打开已有项目，继续上次的工作现场。</p></div><Button className="ml-auto" variant="secondary" onClick={onClose}>返回工作区</Button></div><div className="mt-5 flex gap-2"><input className="h-9 min-w-0 flex-1 rounded border border-input bg-background px-3 text-sm" placeholder="搜索项目" value={query} onChange={event => setQuery(event.target.value)}/><Button size="sm" variant={showArchived ? 'default' : 'secondary'} onClick={() => setShowArchived(value => !value)}>{showArchived ? '查看活跃项目' : '查看已归档'}</Button></div>{error && <p className="mt-3 text-sm text-destructive">{error}</p>}<div className="mt-4 grid gap-3 sm:grid-cols-2">{visible.map(project => <article key={project.id} className="rounded border border-border bg-card p-4">{renaming === project.id ?
