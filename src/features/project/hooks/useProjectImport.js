...
export function useProjectImport({onLoad, onSources, onSelect, onError}) {
    return useCallback(async kind => {
        try {
            ...
            const project = await mangaDeskBridge.createProject({
                name: projectNameFromSource(source),
                sourceDirectory: source.sourcePath || source.directory,
                sources: source.images
            })
            const sources = hydrateSources(source.images, project)
            onLoad(project);
            onSources(sources);
            onSelect(sources[0] || null)
        } catch (error) {
            onError(error instanceof Error ? error.message : '导入失败，请确认 PDF 未加密且文件仍可访问。')
        }
