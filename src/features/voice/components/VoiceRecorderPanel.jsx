...
export function VoiceRecorderPanel({
    ...
    useEffect(() => {
        if (!['recording', 'paused', 'finalizing'].includes(state)) return;
        const warn = event => {
    ...
    const flush = async force => {
        ...
        const output = new Int16Array(frames.current);
        let offset = 0;
        ...
        parts.current = [];
        frames.current = 0;
        const input = {sessionId: session.current, sequence: sequence.current++, pcmBuffer: output.buffer}
    ...
    const stop = async () => {
        try {
            ...
            setError(reason.message || '录音保存失败');
            session.current && await mangaDeskBridge.voice.discard(session.current);
            session.current = null;
            await release();
            setState('error')
        }
    ...
    const togglePause = async () => {
        if (state === 'recording') {
            ...
            await mangaDeskBridge.voice.resume(session.current);
    ...
    const recover = async item => {
        try {
            const take = await mangaDeskBridge.voice.recover(item.id);
            onAddTake(take);
            setRecoverable(items => items.filter(entry => entry.id !== item.id))
        } catch (reason) {
            setError(reason.message || '恢复录音失败')
        }
    }
    const discardRecovery = async item => {
    ...
    return <div className="mt-4 rounded-lg border border-rose-400/30 bg-rose-950/10 p-4">{recoverable.map(item => <div
        ...
        <div className="mt-3 flex items-center gap-2"><select value={deviceId}
                                                              ...
                                                              className="h-8 max-w-56 rounded border border-slate-600 bg-slate-950 px-2 text-xs">
            <option value="default">默认麦克风</option>
            {devices.map(device => <option key={device.deviceId}
                                           value={device.deviceId}>{device.label || '麦克风'}</option>)}</select>
            <div className="h-2 w-24 overflow-hidden rounded bg-slate-800">
                <div className="h-full bg-emerald-400" style={{width: `${Math.min(100, level * 180)}%`}}/>
            </div>
            {state === 'idle' || state === 'review' || state === 'error' ?
                <Button size="sm" onClick={start}><Mic className="h-4 w-4"/>开始录音</Button> : <><Button size="sm"
                                                                                                          variant="secondary"
                                                                                                          onClick={togglePause}>{state === 'paused' ?
                    <Play className="h-4 w-4"/> :
                    <Pause className="h-4 w-4"/>}{state === 'paused' ? '继续' : '暂停'}</Button><Button size="sm"
                                                                                                        variant="destructive"
                                                                                                        disabled={state === 'finalizing'}
                                                                                                        onClick={stop}><Square
                    className="h-4 w-4"/>停止并保存</Button></>}</div>
        {error && <p className="mt-2 text-xs text-red-300">{error}</p>}<label
            className="mt-3 flex items-center gap-2 text-xs text-slate-300"><input type="checkbox"
                                                                                   checked={block.voice?.narrationRequired !== false}
                                                                                   onChange={event => onSetRequired(event.target.checked)}/>此段需要旁白</label>{active &&
            <div className="mt-3 rounded bg-slate-900 p-2 text-xs">
                <div className="mb-1 flex justify-between">
                    <span>当前 Take 裁剪</span><span>{((block.voice?.trimStartMs || 0) / 1000).toFixed(1)}s – {((block.voice?.trimEndMs ?? active.durationMs) / 1000).toFixed(1)}s</span>
                </div>
                <input className="w-full" type="range" min="0" max={Math.max(0, active.durationMs - 200)}
                       value={block.voice?.trimStartMs || 0}
                       onChange={event => onSetTrim(Number(event.target.value), block.voice?.trimEndMs ?? active.durationMs)}/><input
                className="w-full" type="range" min={Math.min(active.durationMs, (block.voice?.trimStartMs || 0) + 200)}
                max={active.durationMs} value={block.voice?.trimEndMs ?? active.durationMs}
                onChange={event => onSetTrim(block.voice?.trimStartMs || 0, Number(event.target.value))}/></div>}
        <div className="mt-3 space-y-1">{(block.voice?.takes || []).map((take, index) => <div key={take.id}
                                                                                              className="flex items-center gap-2 rounded bg-slate-900 px-2 py-1 text-xs">
            <span>Take {index + 1} · {(take.durationMs / 1000).toFixed(1)}s</span>{take.id === active?.id ?
            <b className="text-emerald-300">当前使用</b> :
