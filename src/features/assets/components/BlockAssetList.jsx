...
export function BlockAssetList({assets, sourcesById, selectedAssetId, onSelect, onLocate, onRemove, onMove}) {
    ...
    return <div className="mt-3 space-y-2"><h3 className="text-xs font-bold">已选素材 ({assets.length})</h3>
        {!assets.length &&
            <p className="rounded border border-dashed border-slate-700 p-3 text-center text-xs text-slate-500">按 Enter
                加入整页，或在大图框选后按 Enter</p>}
        {assets.map((asset, index) => <div key={asset.id} draggable
                                           onDragStart={event => event.dataTransfer.setData('application/x-mangadesk-asset', asset.id)}
            ...
            <div className="mt-1 flex items-center gap-1 text-[10px] text-slate-400">
                ...
                size="sm" variant="ghost" disabled={index === assets.length - 1}
                onClick={() => onMove(asset.id, index + 1)} aria-label="后移"><ChevronDown
                className="h-3 w-3"/></Button><Button size="sm" variant="ghost" onClick={() => onRemove(asset.id)}
                                                      aria-label="删除素材"><Trash2 className="h-3 w-3"/></Button></div>
        </div>)}
    </div>
}
