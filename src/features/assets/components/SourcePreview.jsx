...
export function SourcePreview({source, crop, className = 'h-24 w-full'}) {
    if (!source) return <div
        className={`${className} grid place-items-center bg-slate-900 text-xs text-red-300`}>来源不可用</div>
    ...
    return <div className={`${className} overflow-hidden bg-black`}><PageMedia item={source} scale={0.35}
